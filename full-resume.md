...
Solutions Architect, AI/ML Engineer, Data Scientist, and Product Owner with over 7 years of experience at Dell Technologies, driving $100M+ in business value through AI platforms, agentic frameworks, and MLOps. Expert in Kubernetes architecture and leading cross-functional engineering teams through the full software development lifecycle. Ph.D. Sociologist with a deep background in advanced statistical research and multilingual fluency (English, French, Spanish, Arabic), specializing in communicating complex quantitative analyses to non-technical stakeholders and executive leadership.
...
* **Intent Orchestration & Agentic Routing:** Architected a high-throughput **Intent-to-Agent (I2A)** routing engine using semantic similarity and few-shot classification to triage millions of daily chat sessions. Solutioned the integration of multi-tenant state persistence, ensuring that user context was preserved across failovers. This system successfully reduced "incorrect agent" transfers by 40%, with a sustained overall accuracy of 90%.
* **Full-Stack Observability & AIOps:** Implemented a unified telemetry stack (Prometheus/Grafana/Splunk) across 5 core AI products, moving from reactive alerting to proactive anomaly detection. Developed custom "Quality of Response" metrics that tracked LLM and agent relevancy rates and latency percentiles (P95/P99).
...
* **Model Drift & Monitoring:** Developed an automated **ML test harness** for production model drift detection; implemented statistical monitoring (PSI, Kolmogorov-Smirnov) to identify data distribution shifts and trigger automated retraining loops for hardware failure prediction models.
* **DevOps & APIs:** Wrote production-grade Python (FastAPI) and C# (custom search) APIs with automated test suites; containerized front-end web apps and back-end models.
...
* **Agentic AI & Generative Engineering:** Agentic Workflows (ReAct, Planning, Reflection, Self-Correction), Multi-Agent Systems (LangGraph, CrewAI, AutoGen), Model Context Protocol (MCP), Agent to Agent (A2A), Tool Use/Function Calling, RAG 2.0 (GraphRAG, Hybrid Search, Semantic Caching), Prompt Engineering (DSPy, Chain-of-Thought), LLM Evaluation (RAGAS, MLFlow, DeepEval/G-Eval).
* **Solutions Architecture & Platform:** Distributed Systems, Multi-cluster Kubernetes Architecture, High Availability (HA) & Disaster Recovery, Cloud-Native Design, Microservices, Scalable AI Infrastructure, Global Load Balancing, Infrastructure as Code (Terraform, Ansible), Linux Kernel Optimization.
* **AI Security & Governance:** LLM Red Teaming, OWASP Top 10 for LLMs (Prompt Injection, Insecure Output Handling), Adversarial Robustness, PII Anonymization, RBAC for Tool-Use/MCP Servers, CVE scanning/patching, AI Policy (EU AI Act/NIST).
* **MLOps & LLMOps:** Full ML Lifecycle (CI/CD/CT), Model Serving (vLLM, NVIDIA NIM, Kserve, Triton), Observability (Prometheus, Grafana, Splunk, Dynatrace), Feature Stores (Feast), Semantic Caching, Agent Memory (MemGPT), Vector DBs (PGVector, Milvus, Pinecone, Redis), Fine-tuning (LoRA, QLoRA), Quantization, Local Models (Llama.cpp, Ollama).
* **Programming & Frameworks:** Python (LangChain, LlamaIndex, PyTorch, Pandas), SQL (Postgres, Teradata, MSSQL), R (Tidyverse, Shiny), Bash/Powershell, Go (K8s/MCP extensions), C#, Stata, LaTeX.
...
* **Dell Technologies:**
    ...
    * **President’s Award (2020 - $5k):** For Salesforce-integrated ML pipelines and secure k8s-based FastAPI deployment.
    * **President’s Award (2021 - $5k):** For Data Science innovation in predictive hardware solutions.
    * **Game Changer Awards (2022, 2025 x 3):** Architectural and product management contributions ($4.5k total).
...
* **Academic:**
    ...
    * **Phi Beta Kappa:** NYU graduation honors.
    * **Georgetown Graduate School Fellowships:** FLAS and Sabah Scholarships.

---

## Certifications
* **Certification of Professional Achievement in United Nations Studies:** Columbia University / UNDP (SIPA Picker Center, 2009)
